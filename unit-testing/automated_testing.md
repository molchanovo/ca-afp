...
Автоматичне тестування полягає у виконанні тестування програмного коду за рахунок визначення:
- наборів вхідних даних функцій, які необхідно перевірити/протестувати (фактичні параметри);
...
- (і виконання) тестів, тобто функцій, що виконують виклик коду, який необхідно перевірити,
  для визначених вхідних даних, та виклик утиліт порівняння фактичних результатів виконання
  з визначеними очікуваними результатами.

Автоматичне тестування використовується для:
- визначення очікуваної поведінки інтерфейсних функцій (TDD - Test Driven Development);
- перевірки коректності роботи окремих функцій, методів, модулів програми тощо;
...
- перевірки поведінки програми для "граничних" випадків та для випадків некоректних вхідних
  даних;
...
розв'язання інтеграційних проблем. При цьому також виконується запуск автоматичних тестів,
які, в тому числі, мають виявляти можливі проблеми інтеграції різних змін до проєкту.

Реалізації фреймворків для автоматичного (модульного) тестування (unit testing) та
автоматичного створення складних вхідних даних (mocking) наведенні в статтях/документації:
...
## Реалізація автоматичних тестів

Тут розглядається реалізація тестів в контексті л.р. №2, але не обмежується лише цим
контекстом.
...
того результату оцінюється "вручну" - тобто шляхом перегляду і аналізу. Для якихось незначних
завдань цього може бути й достатньо. Проте коли мова заходить про складні програми, то
таке тестувння стає недоречним і вимагатиме багато часу щоразу після внесення змін в код.
...
1. Реалізувати функцію, що на вхід приймає шляхи до двох файлів: вхідна програма та файл з
...
Звісно, можна викликати програму і передавати аргументами перелік вхідних файлів, проте якщо
є певний набір тестів, який має "покрити" логіку цільової функції, і цей набір є сталим,
то можна визначити перелік тестових наборів (або ж виклики тестів) одразу в коді, і зробити
функцію, яка викличе всі тести.
